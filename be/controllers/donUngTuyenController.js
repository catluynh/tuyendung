...
class DonUngTuyenController {
    ...
    async timKiemTheoUngTuyenVien(req, res, next) {
        ...
        await DonUngTuyen.aggregate([
            ...
            { $match: { $and: [{ ungTuyenVien: req.taiKhoan._id }, { trangThai: Enum.TRANG_THAI_DON.DA_UNG_TUYEN }] } },
            { $skip: skip },
            { $limit: limit },
        ])
    ...
    async timKiemTheoNhaTuyenDung1(req, res, next) {
        ...
        const totalAll = await DonUngTuyen.aggregate(
            [
                ...
                { $unwind: "$ungTuyenVien" },
                { $lookup: { from: "taikhoans", localField: "ungTuyenVien._id", foreignField: "_id", as: "taiKhoan" } },
                { $unwind: "$taiKhoan" },
                {
                    $match: {
                        $and: [
