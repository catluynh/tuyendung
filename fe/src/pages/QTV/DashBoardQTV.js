...
const DashBoardQTV = () => {
  ...
  useEffect(async () => {
    if (!socket) {
      const st = io.connect('ec2-54-179-0-126.ap-southeast-1.compute.amazonaws.com:4000')
      setSocket(st)
  ...
  const getDataListFilters = async () => {
    const requestUrl = `ec2-54-179-0-126.ap-southeast-1.compute.amazonaws.com:4000/tinTuyenDungs/soLuongDanhGiaTheoTin?${paramsString}`;
    try {
  ...
  const handleAddButtonClickDetailAccept = async (id) => {
    try {
      const requestUrl = `ec2-54-179-0-126.ap-southeast-1.compute.amazonaws.com:4000/tinTuyenDungs/duyetTin/${id}`;
  ...
  const handleAddButtonClickDetailBlock = async (id) => {
    try {
      const requestUrl = `ec2-54-179-0-126.ap-southeast-1.compute.amazonaws.com:4000/tinTuyenDungs/khoaTin/${id}`;
  ...
  const handleAddButtonClickDetailDeny = async (id) => {
    try {
      const requestUrl = `ec2-54-179-0-126.ap-southeast-1.compute.amazonaws.com:4000/tinTuyenDungs/tuChoiTin/${id}`;
      await axios.patch(requestUrl).then((res) => {
        if (res?.data?.status == "success") {
          setIsSubmit(true);
          toast.success("Cập nhật thành công", {
